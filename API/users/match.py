...
def find_and_notify_matches(target_item: Item, max_distance=2):
    ...
    if target_item.status == "lost":
        matches = get_potential_matches(target_item, opposite_status="found", max_distance=max_distance)
        if matches:
        ...
        potential_items = get_potential_matches(target_item, opposite_status="lost", max_distance=max_distance)
